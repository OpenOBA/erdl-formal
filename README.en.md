...
- **32 of the 34 nodes have SMT encodings**: value / logic / comparison / set / string / existence / quantifier / arithmetic / time / aggregate (comparison and existence dispatch by field type: int / string / bool). `var` (`$`/`$.path` context variables) and `epoch_ms` (date-string→milliseconds parsing) are unused in rules and not yet encoded.
