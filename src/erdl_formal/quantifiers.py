...
def tvl_all(elements):
    """all(elements) — empty → Def(False); else ∧ elements[i] (raw Bool)."""
    if not elements:
        return TVLBool.Def(False)  # E8: all([]) = false (anti-vacuous-truth)
    return TVLBool.Def(And(*elements))


def tvl_any(elements):
    """any(elements) — empty → Def(False); else ∨ elements[i] (raw Bool)."""
    if not elements:
        return TVLBool.Def(False)  # E8: any([]) = false (standard)
    return TVLBool.Def(Or(*elements))


def tvl_none(elements):
    """none(elements) — empty → Def(False); else ¬∨ elements[i] (raw Bool)."""
    if not elements:
        return TVLBool.Def(False)  # E8: none([]) = false (safe deviation)
    return TVLBool.Def(Not(Or(*elements)))
