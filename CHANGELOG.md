...
### Fixed
- **Cross-check number-encoding drift**: `replay/crosscheck-vectors.py` filtered on the stale `rational` value type, so every arithmetic vector was skipped (0/0 consistent); `replay/crosscheck-calendar.py` compared the `date_part` integer against the decimal-string value (false mismatch `got 2026 expected 2026`). Both now align with ERDL expression-layer ER4: `value_type` is `number` and `value` is a decimal string, compared at scale-14 fixed-point numeric equality (trailing-zero insensitive).
...
### Changed
- `resolution_smt.py` docstring and `README.md`: the assurance boundary now states the small-model evidence (n=5/6 UNSAT + witness≤2) and names the induction as the explicit open item, rather than letting "bounded proof" read as more than it is.
