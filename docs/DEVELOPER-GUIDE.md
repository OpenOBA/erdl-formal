...
| `resolution_smt.py` | 裁决层 SMT 全量证明（§7.1 七条性质）| z3 |
...
| resolution_smt | `ResolutionFold` `override_soundness/ring_respect/catch_all_inert_when_explicit/catch_all_then_irrelevant_when_explicit/catch_all_override_irrelevant_when_explicit/emergency_shortcut/workflow_shortcut` | 裁决层 SMT 全量证明 |
...
- 跑全量：`python -m pytest -q`（当前 262 全绿）。
